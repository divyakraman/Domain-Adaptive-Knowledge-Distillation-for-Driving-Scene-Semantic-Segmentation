...
class DRNSeg(nn.Module):
    def __init__(self, classes=19, pretrained_model=None,
                 pretrained=True, use_torch_up=False):
        super(DRNSeg, self).__init__()
        base_model = DRN(BasicBlock, [1, 1, 3, 4, 6, 3, 1, 1], arch='D')
        pretrained_weights = model_zoo.load_url(model_urls['drn-d-38'])
        del pretrained_weights['fc.bias']
        del pretrained_weights['fc.weight']
        base_model.load_state_dict(pretrained_weights)
        #self.base = nn.Sequential(*list(model.children())[:-2])
        self.base = base_model
        self.seg1 = nn.Conv2d(base_model.out_dim, classes,
        ...
        m.weight.data.normal_(0, math.sqrt(2. / n))
        m.bias.data.zero_()
        self.up1 = nn.ConvTranspose2d(classes, classes, 16, stride=8, padding=4,
                                    output_padding=0, groups=classes,
                                    bias=False)
        self.seg2 = nn.Conv2d(base_model.out_dim, classes,
                             kernel_size=1, bias=True) #256 is out dimensions of layer 5: intermediate_out
        m = self.seg2
        n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
        m.weight.data.normal_(0, math.sqrt(2. / n))
        m.bias.data.zero_()
        self.up2 = nn.ConvTranspose2d(classes, classes, 16, stride=8, padding=4,
                                    output_padding=0, groups=classes,
                                    bias=False)
        ...
        '''
    def forward(self, x):
        x, intermediate_out = self.base(x)
        x = self.seg1(x)
        y = self.up(x)
        intermediate_out = self.seg2(intermediate_out)
        intermediate_out = self.up(intermediate_out)
        
        return self.softmax(intermediate_out), self.softmax(y)
    '''
    def optim_parameters(self, memo=None):
        for param in self.base.parameters():
            yield param
        for param in self.seg1.parameters():
            yield param
    '''
